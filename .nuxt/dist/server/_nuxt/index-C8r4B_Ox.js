...
import { _ as __nuxt_component_1 } from "./completed-SzWz5WDV.js";
import "hookable";
      ...
      if (!d.completed) {
        ...
        if (d.text === "01") {
          ...
          ssrRenderList(d.dataList, (item) => {
            _push(`<div class="terms" data-v-789d6944><div class="t-title" data-v-789d6944>${ssrInterpolate(item.toptitle)}</div><div class="textDatas" data-v-789d6944><div class="inner" data-v-789d6944>${item.data ?? ""}</div></div><input type="checkbox"${ssrRenderAttr("id", `checkbox-${item.id}`)} class="custom-checkbox"${ssrIncludeBooleanAttr(Array.isArray(item.checked) ? ssrLooseContain(item.checked, null) : item.checked) ? " checked" : ""} data-v-789d6944><label${ssrRenderAttr("for", `checkbox-${item.id}`)} data-v-789d6944><span class="${ssrRenderClass(item.type === "필수" ? "essential" : "select")}" data-v-789d6944> [${ssrInterpolate(item.type)}]  </span> ${ssrInterpolate(item.name)}</label></div>`);
          });
          _push(`<!--]--></div>`);
        } else {
          _push(`<!---->`);
...
//# sourceMappingURL=index-C8r4B_Ox.js.map
